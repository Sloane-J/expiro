...
export default function LoginPage() {
	...
	const handleSubmit = async (e?: React.FormEvent) => {
		...
		if (isSignUp) {
			if (!phone.trim()) {
				setError(
					"Phone number is required so we can notify you when approved.",
				);
				setLoading(false);
				return;
			}
	...
	const isDisabled =
		loading || !email || !password || (isSignUp && (!phone || !name));
	...
	return (
      ...
      `}</style>
			...
			<main className="w-full max-w-sm relative z-10">
				...
				{/* Toggle */}
				<p
					className="text-center text-sm fu-6"
				...
				<p
					className="text-center text-xs mt-8 fu-6"
					style={{ color: "rgba(128,128,128,0.51)" }}
				>
